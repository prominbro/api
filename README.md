...
Основной endpoint: `https://kfwl.lol/api`
...
```bash
curl https://kfwl.lol/api/health
```
...
```bash
curl -s -X POST https://kfwl.lol/api/v1/happ/decrypt \
...
```bash
curl -s -X POST https://kfwl.lol/api/v1/incy/decrypt \
  -H 'Content-Type: application/json' \
  -d '{"link":"incy://crypt1/..."}'
```
...
```bash
curl -s https://kfwl.lol/api/health
```
...
Production endpoint: `https://kfwl.lol/api`
